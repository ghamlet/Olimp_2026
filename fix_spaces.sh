...
def convert_to_github_spaces(text):
    # 1. Перевод вики-картинок: ![[path/file name.png|Alt]] -> ![Alt](<path/file name.png>)
    ...
    # 2. Перевод вики-заметок: [[path/note name|Label]] -> [Label](<path/note name.md>)
    ...
    # 3. Чистка уже имеющихся обычных Markdown-ссылок (убираем %20, добавляем <>)
    def md_link_clean(match):
    ...
    text = re.sub(r"(!?\[.*?\])\((.*?)\)", md_link_clean, text)
    ...
    for file in files:
        if file.endswith(".md"):
            ...
            try:
                ...
                new_content = convert_to_github_spaces(content)
